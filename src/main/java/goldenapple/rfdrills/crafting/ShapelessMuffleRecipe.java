...
public class ShapelessMuffleRecipe extends ShapelessOreRecipe {
    ...
    public ShapelessMuffleRecipe(ItemStack result) {
        super(
            result,
            result,
            RFDrills.isXULoaded ? new ItemStack(GameRegistry.findItem("ExtraUtilities", "sound_muffler"))
                : "blockCloth");
    }
    ...
    public ItemStack getCraftingResult(InventoryCrafting inventory) {
        ...
        for (int slot = 0; slot < inventory.getSizeInventory(); slot++) {
            if (inventory.getStackInSlot(slot) != null && inventory.getStackInSlot(slot)
                .getItem() == getRecipeOutput().getItem())
