...
public class WailaCompat implements IWailaDataProvider {
    ...
    public List<String> getWailaBody(ItemStack stack, List<String> tooltip, IWailaDataAccessor accessor,
        ...
        if (accessor.getPlayer()
            ...
            || !(accessor.getPlayer()
                ...
                .getItem() instanceof IEnergyTool))
            return tooltip;

        ItemStack equipStack = accessor.getPlayer()
            .getCurrentEquippedItem();
        ...
        if (equipStack.getItem() instanceof IEnergyTool) {
            if (config.getConfig("rfdrills.waila_rf")) {
                ...
                tooltip.add(
                    StringHelper.writeEnergyPerBlockInfo(energyTool.getEnergyPerUse(equipStack, block, meta), harvest));

            }
            if (config.getConfig("rfdrills.waila_mode") && (accessor.getPlayer()
                .isSneaking() || !(config.getConfig("rfdrills.waila_mode.sneakingonly")))) {
                if (!energyTool.writeModeInfo(equipStack)
                    .isEmpty()) {
    ...
    public static void init(IWailaRegistrar registrar) {
        ...
        registrar.addConfig(
            Reference.MOD_NAME,
            "rfdrills.waila_mode.sneakingonly",
            "option.rfdrills.waila_mode.sneakingonly");
