...
public class PlayerRenderHandler {
    ...
    public void onPlayerRender(RenderPlayerEvent.Pre event) { // handles 3rd person rendering
        if (ConfigHandler.animationType == 1) {
            ...
            if (equippedStack.getItem() instanceof IEnergyTool && equippedStack.getItem()
                .getItemUseAction(equippedStack) == EnumAction.bow) {
    ...
    @SubscribeEvent
    public void onClientTick(TickEvent.PlayerTickEvent event) { // handles both 1st and 3rd person rendering, but in a
                                                                // cheatier way
        if (ConfigHandler.animationType == 2 && event.phase == TickEvent.Phase.END) {
            ...
            if (equippedStack.getItem() instanceof IEnergyTool && equippedStack.getItem()
                ...
                if (event.player.isSwingInProgress) {
                    event.player.setItemInUse(equippedStack, event.player.ticksExisted); // a bit of a hack here. I need
                                                                                         // a variable that would
                                                                                         // increase every tick,
                                                                                         // otherwise the "bow shaking"
                                                                                         // effect won't happen.
                                                                                         // player.ticksExisted works
                                                                                         // for that.
